    ...
    <div className="min-h-screen bg-[#0A0A0A] text-[#E5E7EB] font-sans selection:bg-[#3D2616]">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#0A0A0A]/90 backdrop-blur-md px-6 md:px-12 py-5 flex justify-between items-center">
        <h1 className="text-2xl font-black tracking-tighter text-white">
          LMS <span className="text-[#A67C52]">PRO</span>
        </h1>
      ...
      {/* Roles Section - Updated Click Logic */}
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {roles.map((role, i) => (
            <div 
              key={i} 
              onClick={() => navigate('/login', { state: { selectedRole: role.title } })}
      ...
      {/* Featured Section - Your Original Design & Effects Unchanged */}
      <section className="max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
        <div className="flex justify-between items-center mb-16">
          ...
          <button 
            type="button" 
            onClick={() => navigate('/catalog')}
            className="text-[#A67C52] font-bold text-sm tracking-widest uppercase hover:underline flex items-center gap-2 cursor-pointer"
          >
