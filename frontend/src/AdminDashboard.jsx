import { Book, LogOut, Users, PlusCircle, LayoutDashboard, Shield, CheckCircle, X, Image as ImageIcon, Trash2, Star, Menu, Edit3, UserMinus, Search } from 'lucide-react';
...
const handleAddBook = async (e) => {
  ...
  if (result.success) {
        ...
        fetchData();
    } else { alert(result.message || "Failed to add book"); }
...
<main className="flex-1 p-6 md:p-12 overflow-y-auto max-h-screen">
  <header className="flex items-center justify-between mb-16">
    <div className="flex items-center gap-6">
        <img src={`http://localhost/library-management/frontend/public/profiles/${adminData?.profile_pic || 'tas_admin.jpg'}`}className="w-20 h-20 rounded-3xl border-2 border-[#D4A373]/30 object-cover shadow-2xl"alt="Profile" onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${adminData?.name || 'Admin'}&background=D4A373&color=fff` }} />
  <div>
...
{activeTab === 'dashboard' ? (
  <>
<div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">{[{ label: 'Total Assets', value: stats.total_books, icon: Book },{ label: 'Members', value: stats.total_users, icon: Users },{ label: 'Active Issues', value: stats.active_issues, icon: CheckCircle },].map((item, i) => (
...
<section className="bg-[#121620]/40 border border-gray-800/50 rounded-3xl overflow-hidden shadow-2xl">
  <div className="p-10 border-b border-gray-800/50 bg-white/5 flex justify-between items-center">
    <h3 className="text-xl font-bold text-[#D4A373] tracking-[0.2em] uppercase italic">Recent Inventory</h3>
      {searchTerm && <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Found: {filteredBooks.length}</span>}
  </div>
...
<tbody className="divide-y divide-gray-800/20">{filteredBooks.length > 0 ? filteredBooks.map((book) => (
  <tr key={book.book_id} className="hover:bg-white/5 transition-all group">
  <td className="p-8">
    <img src={`http://localhost/library-management/backend/api/uploads/${book.image_url}`} className="w-12 h-16 object-cover rounded-xl border border-gray-800 shadow-xl" alt="book"onError={(e) => { e.target.onerror = null; e.target.src = 'https://via.placeholder.com/150?text=No+Cover'; }} />
  </td>
  <td className="p-8">
...
<td className="p-8 text-center"><span className="bg-[#0A0D14] text-[#D4A373] text-[9px] px-3 py-1.5 rounded-full font-black border border-gray-800 uppercase">{categories[book.category_id] || 'N/A'}
</span>
</td>
...
<td className="p-8 text-right flex justify-end gap-3">
  ...
  <tr><td colSpan="6" className="p-20 text-center text-gray-600 font-bold uppercase tracking-widest">No books found matching your search</td></tr>
  )}
</tbody>
</table>
</div>
