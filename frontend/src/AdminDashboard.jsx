...
const AdminDashboard = () => {
  ...
  const handleAddBook = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost/library-management/backend/api/add_book.php", { 
        method: "POST",
        body: formData
      });
                      ...
                      <tr key={book.book_id} className="hover:bg-white/5 transition-all group">
                        ...
                        <td className="p-8 text-center">
                          {String(book.is_featured) === '1' ? (
                            <Star className="text-yellow-500 fill-yellow-500 mx-auto drop-shadow-[0_0_8px_rgba(234,179,8,0.5)]" size={18} />
                          ) : (
                            <Star className="text-gray-800 mx-auto opacity-20" size={18} />
                          )}
                        </td>
          ...
          <Modal title={isAddModalOpen ? "Add New Book" : "Edit Book"} close={() => { setIsAddModalOpen(false); setIsEditModalOpen(false); setEditingBook(null); setSelectedFile(null); }}>
            <form onSubmit={isAddModalOpen ? handleAddBook : handleUpdateBook} className="space-y-5">
              ...
              <button type="submit" className="modal-submit-btn">
                {isAddModalOpen ? "Save Book" : "Update Details"}
              </button>
            </form>
          </Modal>
        )}
